    ...
    def format_page(self, menu, entries):
        ...
        embed = CustomEmbed(
            title=(
                ...
                f"[{' | '.join([alias for alias in self.cog.aliases ])}]"
                if getattr(self.cog, "aliases", None) is not None
                else ""
            )
        )
        ...
        for index, command in enumerate(entries, start=offset):
            ...
            embed.add_field(
                name=f"**{str(command)}** [{' | '.join(alias for alias in command.aliases)}]"
                if command.aliases
                else f"**{str(command)}**",
                value=(command.help or "No help given"),
                inline=False,
            )
...
class CustomHelp(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        filtered_commands = [
            {key: await self.filter_commands(value)}
            for key, value in mapping.items()
            if len(value) != 0
        ]
        clean_prefix = self.context.clean_prefix
