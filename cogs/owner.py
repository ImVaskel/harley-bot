...
class Owner(commands.Cog, command_attrs=dict(hidden=True)):
    ...
    async def reload(self, ctx, cog, **flags):
        ...
        try:
            ...
            await ctx.reply(embed = CustomEmbed(
                description=f"Reload of `{cog}` successful."
            ))
    ...
    async def reload_all(self, ctx):
        ...
        embed.add_field(name="Unsuccessful",
                        value="\t".join(unsuccessful) or "None", inline=False)

        await ctx.send(embed=embed)
    
    @flags.add_flag("--iterate", action="store_true", help="Iterates through the last X messages in the channel instead.")
    @commands.is_owner()
    @dev.command(aliases = (
        "clear", "cleanup"
