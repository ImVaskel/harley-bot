...
class Fun(commands.Cog):
    ...
    async def apod(self, ctx, date = None):
        ...
        url = f"https://api.nasa.gov/planetary/apod?date={date}&api_key={self.bot.config['nasa']}" if date is not None else f"https://api.nasa.gov/planetary/apod?api_key={self.bot.config['nasa']}"
    ...
    async def xkcd(self, ctx, comic = None):
        ...
        url = f"http://xkcd.com/{comic}/info.0.json" if comic else "http://xkcd.com/info.0.json"
