...
class Moderator(commands.Cog):
    ...
    async def perform_unmute(self, *, member: discord.Member, role: discord.Role, when: datetime.datetime,
    ...
    async def mute_pull(self):
        ...
        for guild in self.bot.guilds:
            records: list[asyncpg.Record] = await self.bot.db.fetch(query, guild.id, now, later)
            ...
            for record in records:
                ...
                role: discord.Role = guild.get_role(self.bot.cache[guild.id]['muted_role'])
                ...
                task = self.bot.loop.create_task(self.perform_unmute(member=member, role=role, when=when))
    ...
    @flags.add_flag("--force", action="store_true", help="Forces a kick without notifying the user.")
    @flags.add_flag("--reason", help="The reason for the kick.")
    @commands.command(cls=flags.FlagCommand)
    @commands.has_guild_permissions(kick_members=True)
    @commands.bot_has_guild_permissions(kick_members=True)
    async def kick(self, ctx, member: HierarchyMemberConverter, **flags):

        notified = None
        reason = flags.get("reason", "None Given")

        if flags.get('force') is True:
            try:
                ...
                return await ctx.send(embed=CustomEmbed(
                    description="I was unable to kick the user for whatever reason."
                ))

        try:
            await member.send(embed=CustomEmbed(
                title="Kicked",
    ...
    @flags.add_flag("--force", action="store_true", help="Forces a ban without notifying the user.")
    @flags.add_flag("--reason", help="The reason for the ban.")
    @flags.add_flag("--delete", type=int, default=1,
                    help="The number of days worth of messages to delete from the member. Max is 7.")
    @commands.command(cls=flags.FlagCommand)
    @commands.has_guild_permissions(ban_members=True)
    @commands.bot_has_guild_permissions(ban_members=True)
    async def ban(self, ctx, member: HierarchyMemberConverter, **flags):
        ...
        if flags.get('force') is True:
            try:
                await member.ban(reason=reason, delete_message_days=flags.get("delete"))
                notified = False
                await ctx.reply(embed=CustomEmbed(
                    title="Banned User",
    ...
    async def softban(self, ctx, member: HierarchyMemberConverter, **flags):
        ...
        await ctx.reply(
            embed=CustomEmbed(
                ...
                description=f"Softbanned user {member}."
            )
        )
    ...
    async def mute(self, ctx, member: HierarchyMemberConverter, time: TimeConverter, *, reason="None Given"):
        """
        ...
        if muted_role in member.roles:
            ...
            await ctx.db.execute(
                "DELETE FROM mutes WHERE guildid = $1 and userid = $2", ctx.guild.id, member.id
            )
        sleep = (time - datetime.datetime.utcnow()).total_seconds()
        if sleep <= 1800:
            task = self.bot.loop.create_task(
                self.perform_unmute(member=member, role=muted_role, when=time, record=None))
            task.add_done_callback(self.unmute_error)

        await ctx.db.execute(
            "INSERT INTO mutes(userid, guildid, starttime, endtime, reason) VALUES($1, $2, $3, $4, $5)", member.id,
            ctx.guild.id, datetime.datetime.utcnow(), time, reason
        )

        await member.add_roles(muted_role, reason=f"Mute done by {ctx.author} [{ctx.author.id}]")
    ...
    async def unmute(self, ctx, member: HierarchyMemberConverter):
        ...
        if muted_role in member.roles:
            await member.remove_roles(muted_role, reason=f"Unmute done by {ctx.author} [{ctx.author.id}]")
            await ctx.db.execute("DELETE FROM mutes WHERE userid = $1 and guildid = $2", member.id, ctx.guild.id)
            await ctx.reply(embed=CustomEmbed(
